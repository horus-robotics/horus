...
impl Node for StateMachineNode {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        ...
        self.state = match self.state {
            ...
            RobotState::Moving => {
                if obstacle {
                    self.cmd_pub.send(CmdVel::zero(), ctx).ok();  // Stop
                    RobotState::ObstacleDetected
                } else {
                    self.cmd_pub.send(CmdVel::new(1.0, 0.0), ctx).ok();  // Forward
                    RobotState::Moving
                }
            }
            ...
            RobotState::Rotating => {
                self.cmd_pub.send(CmdVel::new(0.0, 0.5), ctx).ok();  // Rotate
...
impl Node for EmergencyStopNode {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        ...
        if let Some(min_dist) = self.lidar_sub.recv(ctx) {
        ...
        if should_stop != self.estop_active {
            self.estop_pub.send(should_stop, ctx).ok();
    ...
    fn shutdown(&mut self, ctx: &mut NodeInfo) -> Result<(), String> {
        ...
        self.estop_pub.send(true, None).ok();
        ctx.log_warn("Emergency stop system offline");
        Ok(())
    }
}
...
impl Node for MotorController {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        ...
        if let Some(estop) = self.estop_sub.recv(ctx) {
            if estop != self.estop_active {
                self.estop_active = estop;
                if let Some(ctx) = ctx {
                    if estop {
                        ctx.log_warn("Motors DISABLED - emergency stop active");
                    } else {
        ...
        if self.estop_active {
            self.motor_pub.send(CmdVel::zero(), ctx).ok();
            return;
        }

        // Process normal commands
        if let Some(cmd) = self.cmd_sub.recv(ctx) {
            self.motor_pub.send(cmd, ctx).ok();
    ...
    fn shutdown(&mut self, ctx: &mut NodeInfo) -> Result<(), String> {
        // Stop motors
        self.motor_pub.send(CmdVel::zero(), None).ok();
...
impl Node for TempSensor {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        ...
        if let Some(ctx) = ctx {
            ctx.log_debug(&format!("Rust: {:.2}°C", temp));
        }
...
- [Design Patterns](/design-patterns) - Common robotics patterns (coming soon)
- [Performance Optimization](/performance) - Tune for maximum throughput
- [Python Bindings Reference](/python-bindings) - Complete Python API
- [Remote Deployment](/remote-deployment) - Deploy to robots
- [Package Management](/package-management) - Share and reuse nodes

Need help?
- [Troubleshooting Runtime Errors](/troubleshooting-runtime) - Debug common issues
- [Dashboard](/dashboard) - Monitor your system in real-time
