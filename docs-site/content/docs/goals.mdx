...
**Sub-Microsecond Communication**: Achieve 312ns-481ns latency for robotics messages, enabling real-time control loops at 1kHz+ frequencies
...
**Unified High-Level Workflow**: Single command for build, run, and deployment across Rust, Python, and C++ with minimal configuration (simple horus.yaml vs complex ROS XML)
...
- 312ns (Link) / ~500ns (Hub) for CmdVel (16B messages)
...
let mut scheduler = Scheduler::new();
let mut pid = PidControllerNode::new(1.0, 0.1, 0.05)?;
// Customize if needed
...
let mut scheduler = Scheduler::new();
scheduler.add(Box::new(ImuNode::new("/dev/i2c-1", "mpu6050")?), 0, Some(true));
scheduler.add(Box::new(LidarNode::new("/dev/ttyUSB0")?), 0, Some(true));
scheduler.add(Box::new(PidControllerNode::new(1.0, 0.1, 0.05)?), 1, Some(true));
...
- **Performance**: 312ns (Link) / ~500ns (Hub) CmdVel latency for real-time control
...
- **Performance**: 312ns-481ns message passing enables sub-millisecond end-to-end latency
