...
scheduler.add(Box::new(BarometerNode::new()?), 0, Some(true));
scheduler.add(Box::new(FlightController::new()?), 1, Some(true));
scheduler.add(Box::new(MotorController::new()?), 2, Some(true));
scheduler.run()?;
```
