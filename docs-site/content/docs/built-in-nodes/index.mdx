...
The `horus_library` includes a comprehensive collection of **32 production-ready nodes** and **hardware drivers** for building robotics applications. All hardware nodes support automatic **simulation fallback** when hardware is unavailable.
...
1. **Simple construction** - `NodeType::new()?` with intelligent defaults
2. **Clear configuration** - `set_*()` methods with named parameters (no magic numbers)
