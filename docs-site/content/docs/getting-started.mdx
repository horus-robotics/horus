...
**Macro-Based Implementation** (covered in [node! Macro Guide](/docs/node-macro)):
```rust
node! {
    SensorNode {
...
impl Node for TemperatureDisplay {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        if let Some(temp) = self.subscriber.recv(ctx) {
            println!("🌡️  Current temperature: {:.1}°C", temp);
        }
    }
}
...
🌡️  Current temperature: 20.1°C
🌡️  Current temperature: 20.2°C
🌡️  Current temperature: 20.3°C
```
...
3. [CLI Reference](/docs/cli-reference) - Learn the commands

📚 Core Concepts (2 hours)
4. [Nodes](/docs/core-concepts-nodes) - Build components
5. [Hub](/docs/core-concepts-hub) - Send and receive messages
6. [Scheduler](/docs/core-concepts-scheduler) - Run your application

🎯 Practical Features (3 hours)
7. [node! Macro](/docs/node-macro) - Eliminate boilerplate
8. [Message Types](/docs/message-types) - Work with data safely
9. [Dashboard](/docs/dashboard) - Monitor and debug

🌟 Advanced (When you need it)
10. [Multi-Language](/docs/multi-language) - Python and C
11. [Performance](/docs/performance) - Optimize for speed
12. [Examples](/docs/examples) - Real projects
...
3. **[See Examples →](/docs/examples)**
   Learn from real projects

**Questions?** Check out the [CLI Reference](/docs/cli-reference) or [Architecture](/docs/architecture) pages.
