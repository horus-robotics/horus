...
- When to use different priority levels (0=Critical for emergency stops, 4=Background for logging)
...
// ⚠️ Cannot achieve - 100 Hz+ control
...
**Roadmap:** Configurable per-node tick rates and removal of the global 60 FPS limit are planned for future releases. See [Roadmap](/roadmap) for details.
...
scheduler.add(Box::new(safety_monitor), 0, Some(true));    // Runs 1st (Critical)
scheduler.add(Box::new(controller), 1, Some(true));         // Runs 2nd (High)
scheduler.add(Box::new(sensor), 2, Some(true));             // Runs 3rd (Normal)
scheduler.add(Box::new(logger), 4, Some(true));             // Runs 4th (Background)
...
When you call `run()`, the scheduler first initializes all nodes:
...
```
[12:34:56.789] [IPC: 481ns | Tick: 12μs] PublisherNode --PUB--> 'cmd_vel' = 1.5
```
...
- Learn about [Message Types](/message-types) for communication
- Explore [Examples](/examples) for complete applications
- Read the [API Reference](/api-scheduler) for detailed documentation
