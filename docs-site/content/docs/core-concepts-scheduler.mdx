...
for node_metrics in metrics {
    ...
    println!("  Avg tick: {}μs", node_metrics.avg_tick_duration_us);
...
```
[12:34:56.789] [IPC: 366ns | Tick: 12μs] PublisherNode --PUB--> 'cmd_vel' = 1.5
```
...
fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
    if let Some(ctx) = ctx {
        ...
        if avg_us > 1000 {
            ctx.log_warning(&format!("Slow tick: {}μs", avg_us));
        }
    }
}
```
...
- Learn about [Message Types](https://docs.horus-registry.dev/docs/message-types) for communication
- Explore [Examples](https://docs.horus-registry.dev/docs/examples) for complete applications
- Read the [API Reference](https://docs.horus-registry.dev/docs/api-scheduler) for detailed documentation
