...
impl Node for TemperatureSensor {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        // Publish temperature
        self.temp_pub.send(self.reading, ctx).ok();
    }
...
mod tests {
    ...
    fn test_sensor_init_lifecycle() {
        ...
        let mut ctx = NodeInfo::new("TemperatureSensor", true);
...
fn test_init() {
    ...
    let mut ctx = NodeInfo::new("MyNode", true);
...
impl Node for TemperatureProcessor {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        if let Some(celsius) = self.input_sub.recv(ctx) {
            let fahrenheit = self.process_temperature(celsius);
            self.output_pub.send(fahrenheit, ctx).ok();
        }
    }
}
...
impl Node for GeneratorNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        self.counter += 1;
        self.output_pub.send(self.counter, ctx).ok();
    }
}
...
impl Node for DoublerNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        if let Some(n) = self.input_sub.recv(ctx) {
            self.output_pub.send(n * 2, ctx).ok();
        }
    }
}
...
- **[Examples](/examples)** - See complete tested applications
- **[Core Concepts - Nodes](/core-concepts-nodes)** - Understand node lifecycle
- **[Dashboard](/dashboard)** - Debug with visual monitoring
