...
|-----------|-----------|------------|-----------|
| **HORUS Link (SPSC)** | **312 ns** | **~400 ns** | **~900 ns** |
| **HORUS Hub (MPMC)** | **481 ns** | **~620 ns** | **~1.4 μs** |
| ROS2 (DDS) | 50-100 μs | 100-500 μs | 1-10 ms |
| ROS2 (FastDDS) | 20-50 μs | 50-200 μs | 500 μs - 5 ms |

**Performance Advantage**: HORUS is **64-320x faster** than ROS2 for typical message sizes.
...
|-------------|--------------|-------------|--------------|---------|
| 16 B | CmdVel | ~500 ns | 312 ns | **100-320x faster** |
...
```
  ...
  CmdVel (Motor Control Command)
    ...
    Latency (avg): ~500 ns (Hub) / 312 ns (Link)
...
|-------------|-----------|--------------|-------------|------|---------|
| Motor control | 1000 Hz | 312 ns | ~500 ns | 50 μs | **100-160x** |
...
**Link (SPSC) - Point-to-Point:**
- **312 ns** - CmdVel (motor control)
- **~400 ns** - IMU (sensor fusion)
- **~900 ns** - LaserScan (2D lidar)
- **~600 ns** - Odometry (localization)
- **~120 μs** - PointCloud with 10K points

**Hub (MPMC) - Pub/Sub:**
- **~500 ns** - CmdVel (motor control)
- **~940 ns** - IMU (sensor fusion)
