...
Returns the execution priority for this node. Lower numbers = higher priority.
...
**Returns**: `u32` priority value. Common values:
- `0` = Critical (highest priority)
- `10` = High
- `50` = Normal (default)
- `80` = Low
- `100` = Background (lowest priority)
...
impl Node for TemperatureSensor {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        // Publish temperature
        self.temperature_pub.send(self.reading, &mut ctx).ok();
    }
...
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    ...
    if let Some(input) = self.keyboard_sub.recv(None) {
        ...
        self.state_pub.send(state, &mut ctx).ok();
    }
}
```
...
- **Advanced**: Use `&mut ctx` for both `recv()` and `send()` when you need IPC timing logs for both
...
Nodes execute in priority order each tick. Use priorities to ensure critical nodes run first. You can use any u32 value (0-4294967295) for fine-grained control.
