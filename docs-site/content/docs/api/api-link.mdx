...
**Performance advantage**: Link is ~1.94x faster than Hub (248ns vs 481ns round-trip)
...
**Performance**: ~89ns for send operation alone, ~248ns full round-trip
...
// With automatic logging (recommended)
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    if let Some(value) = link.recv(&mut ctx) {
        self.process(value);
    }
}
```
...
impl Node for MotorDriver {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        // Receive command
        if let Some(cmd) = self.cmd_input.recv(&mut ctx) {
            self.apply_voltage(cmd.voltage);
        }

        // Send encoder feedback
        let reading = self.read_encoder();
        self.encoder_output.send(reading, &mut ctx).ok();
    }
}
...
impl Node for Controller {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        // Receive feedback
        if let Some(reading) = self.encoder_input.recv(&mut ctx) {
            // PD control
            let cmd = self.compute_control(reading);

            // Send command
            self.cmd_output.send(cmd, &mut ctx).ok();
        }
    }
}
```
...
|--------------|--------------|-------------|----------------|
| 16B (CmdVel) | 248ns | 481ns | **1.94x faster** |
| 304B (IMU) | ~400ns | ~940ns | **2.35x faster** |
| 1.5KB (LaserScan) | ~900ns | ~2.2µs | **2.44x faster** |

**Latency breakdown**:
- Send operation: ~89ns
- Atomic ordering: ~50ns
- Receive operation: ~109ns
- Total round-trip: ~248ns

### Comparison with ROS 2

| System | Latency | Speedup |
|--------|---------|---------|
| ROS 2 Intra-process | 50-100µs | **200-400x** |
| ROS 2 Inter-process | 200-500µs | **800-2000x** |
| **HORUS Link** | **0.248µs** | **Baseline** |
| **HORUS Hub** | **0.481µs** | 0.52x |
...
impl Node for SensorNode {
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        if (value - self.last_sent).abs() > 0.01 {
            self.output.send(value, &mut ctx).ok();
...
- [Benchmarks](/performance/benchmarks) - Performance analysis
