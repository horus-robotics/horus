---
title: Hub API Reference
description: Complete API reference for the Hub pub/sub system
order: 411
---

# Hub API Reference

The `Hub<T>` provides ultra-low latency pub/sub communication through shared memory.

## Creating a Hub

### `Hub::new(topic_name)`

Create a new hub for publishing and subscribing to a topic.

```rust
pub fn new(topic_name: &str) -> Result<Self>
```

**Parameters**:
- `topic_name`: Name of the topic (string)

**Returns**: `Result<Hub<T>, HorusError>`

**Example**:
```rust
use horus::prelude::*;

let velocity_hub: Hub<f32> = Hub::new("cmd_vel")?;
```

**Type Constraints**:
- `T` must implement `Clone + Debug + Send + Sync`

### `Hub::new_with_capacity(topic_name, capacity)`

Create a hub with custom message capacity.

```rust
pub fn new_with_capacity(
    topic_name: &str,
    capacity: usize
) -> Result<Self>
```

**Parameters**:
- `topic_name`: Name of the topic
- `capacity`: Maximum number of messages to buffer (not bytes!)

**Returns**: `Result<Hub<T>, HorusError>`

**Example**:
```rust
// Create hub with buffer for 10 messages
let hub: Hub<f32> = Hub::new_with_capacity("data", 10)?;
```

**Understanding Capacity vs Memory:**

Capacity specifies the **number of messages**, not bytes. Actual memory usage:

```rust
memory_bytes = capacity × sizeof(T) + overhead
```

**Examples:**
```rust
// 1000 × 4 bytes = 4 KB
Hub::<f32>::new_with_capacity("float_data", 1000)?;

// 1000 × 304 bytes = 304 KB
Hub::<Imu>::new_with_capacity("imu", 1000)?;

// 100 × 1536 bytes = 150 KB
Hub::<LaserScan>::new_with_capacity("scan", 100)?;
```

**Calculating from Memory Budget:**

If you have a specific memory budget (e.g., 50 MB):

```rust
use std::mem::size_of;

let target_mb = 50;
let target_bytes = target_mb * 1024 * 1024;
let capacity = target_bytes / size_of::<PointCloud>();

Hub::<PointCloud>::new_with_capacity("points", capacity)?;
```

**System Limits:**
- Max capacity: 1,000,000 messages
- Max element size: 1 MB per message
- Max total size: 100 MB per topic

See [Shared Memory Guide](/core-concepts-shared-memory#understanding-capacity-vs-memory-size) for detailed memory planning.

### `Hub::new_global(topic_name)`

Create a global hub accessible across all sessions (not scoped to session_id).

```rust
pub fn new_global(topic_name: &str) -> Result<Self>
```

**Parameters**:
- `topic_name`: Name of the global topic

**Returns**: `Result<Hub<T>, HorusError>`

**Example**:
```rust
use horus::prelude::*;

// Session-scoped hub (default) - only visible within same session_id
let session_hub: Hub<f32> = Hub::new("sensor_data")?;

// Global hub - visible across ALL sessions
let global_hub: Hub<f32> = Hub::new_global("system_status")?;
```

**When to Use:**
- **System-wide topics** shared across multiple applications
- **Cross-session communication** between unrelated processes
- **Monitoring/logging** systems that aggregate data from all sessions

**When NOT to Use:**
- **Single application** - use `Hub::new()` (faster, session-isolated)
- **Multi-process app with session_id** - use `Hub::new()` (automatic session sharing)

**Shared Memory Location:**
```
Session-scoped: /dev/shm/horus/sessions/{session_id}/topics/{topic_name}
Global:         /dev/shm/horus/global/topics/{topic_name}
```

**See Also**: [Configuration Reference - session_id](/configuration#session_id-optional) and [Cross-Session Communication](/core-concepts-shared-memory#cross-session-communication)

### `Hub::new_global_with_capacity(topic_name, capacity)`

Create a global hub with custom capacity.

```rust
pub fn new_global_with_capacity(
    topic_name: &str,
    capacity: usize
) -> Result<Self>
```

**Parameters**:
- `topic_name`: Name of the global topic
- `capacity`: Maximum number of messages to buffer

**Returns**: `Result<Hub<T>, HorusError>`

**Example**:
```rust
// Global hub with 10,000 message capacity
let hub: Hub<SystemMetrics> = Hub::new_global_with_capacity("metrics", 10_000)?;
```

## Publishing Messages

### `send(msg, ctx)`

Send a message to all subscribers on this topic.

```rust
pub fn send(&self, msg: T, ctx: Option<&mut NodeInfo>) -> Result<(), T>
```

**Parameters**:
- `msg`: Message to send (moved, then cloned internally)
- `ctx`: Optional NodeInfo for automatic logging

**Returns**:
- `Ok(())`: Message sent successfully
- `Err(msg)`: Failed to send (returns original message)

**Performance**: 437ns for small messages (16B), scales linearly with size

**Example**:
```rust
// Without logging
hub.send(42.0, None).ok();

// With automatic logging (recommended)
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    hub.send(42.0, &mut ctx).ok();
    // Automatically logs: [12:34:56.789] [IPC: 437ns] MyNode --PUB--> 'topic' = 42.0
}
```

**Error Handling**:
```rust
match hub.send(data, &mut ctx) {
    Ok(()) => {
        // Message sent successfully
    }
    Err(original_data) => {
        // Failed - shared memory full or unavailable
        // original_data contains the message that couldn't be sent
    }
}
```

## Subscribing to Messages

### `recv(ctx)`

Receive a message from the topic (non-blocking).

```rust
pub fn recv(&self, ctx: Option<&mut NodeInfo>) -> Option<T>
```

**Parameters**:
- `ctx`: Optional NodeInfo for automatic logging

**Returns**:
- `Some(msg)`: Message received
- `None`: No message available (not an error)

**Performance**: 437ns for small messages (16B)

**Example**:
```rust
// Without logging
if let Some(velocity) = hub.recv(None) {
    println!("Received: {}", velocity);
}

// With automatic logging (recommended)
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    if let Some(velocity) = hub.recv(&mut ctx) {
        // Automatically logs: [12:34:56.789] [IPC: 142ns] MyNode <--SUB-- 'topic' = 42.0
        self.process(velocity);
    }
}
```

**Best Practices**:
- Always check `recv()` every tick for responsive nodes
- `None` is normal - means no new messages
- Messages are cloned when received

## Monitoring Methods

### `get_connection_state()`

Get current connection state (lock-free).

```rust
pub fn get_connection_state(&self) -> ConnectionState
```

**Returns**: `ConnectionState` enum

**Connection States**:
- `Disconnected` - Not connected
- `Connecting` - Establishing connection
- `Connected` - Ready for communication
- `Reconnecting` - Attempting to reconnect
- `Failed` - Connection failed

**Example**:
```rust
if hub.get_connection_state() == ConnectionState::Connected {
    hub.send(data, &mut ctx).ok();
}
```

### `get_metrics()`

Get performance metrics snapshot (lock-free).

```rust
pub fn get_metrics(&self) -> HubMetrics
```

**Returns**: `HubMetrics` struct with:
- `messages_sent: u64`
- `messages_received: u64`
- `send_failures: u64`
- `recv_failures: u64`

**Example**:
```rust
let metrics = hub.get_metrics();
println!("Sent: {}, Received: {}",
         metrics.messages_sent,
         metrics.messages_received);
```

### `get_topic_name()`

Get the topic name for this hub.

```rust
pub fn get_topic_name(&self) -> &str
```

**Returns**: Topic name as string slice

**Example**:
```rust
println!("Topic: {}", hub.get_topic_name());
```

## Valid Message Types

### Primitives

```rust
Hub::<f32>::new("float")?;
Hub::<f64>::new("double")?;
Hub::<i32>::new("int")?;
Hub::<u32>::new("uint")?;
Hub::<bool>::new("bool")?;
Hub::<String>::new("string")?;
```

### Arrays

```rust
Hub::<[f32; 3]>::new("position")?;
Hub::<[u8; 1024]>::new("buffer")?;
```

### Structs

Must implement `Clone + Debug`:

```rust
#[derive(Clone, Debug)]
struct RobotPose {
    x: f32,
    y: f32,
    theta: f32,
}

let pose_hub: Hub<RobotPose> = Hub::new("pose")?;
```

### Standard Messages

HORUS provides pre-defined message types:

```rust
use horus::prelude::*;
use horus_library::messages::{CmdVel, LaserScan, Imu};

let cmd_hub: Hub<CmdVel> = Hub::new("cmd_vel")?;
let scan_hub: Hub<LaserScan> = Hub::new("scan")?;
let imu_hub: Hub<Imu> = Hub::new("imu")?;
```

## Communication Patterns

> **Note**: Examples below focus on communication patterns. For full working examples with constructors, see [Core Concepts: Hub](/core-concepts-hub) and [Examples](/examples).

### One-to-One

Single publisher, single subscriber:

```rust
// Publisher node
struct PublisherNode {
    data_pub: Hub<f32>,
}

impl PublisherNode {
    fn new() -> Result<Self> {
        Ok(Self {
            data_pub: Hub::new("data")?,
        })
    }
}

impl Node for PublisherNode {
    fn name(&self) -> &'static str {
        "PublisherNode"
    }

    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        self.data_pub.send(42.0, &mut ctx).ok();
    }
}

// Subscriber node
struct SubscriberNode {
    data_sub: Hub<f32>,
}

impl SubscriberNode {
    fn new() -> Result<Self> {
        Ok(Self {
            data_sub: Hub::new("data")?,
        })
    }
}

impl Node for SubscriberNode {
    fn name(&self) -> &'static str {
        "SubscriberNode"
    }

    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        if let Some(data) = self.data_sub.recv(&mut ctx) {
            println!("Got: {}", data);
        }
    }
}
```

### One-to-Many (Broadcast)

Single publisher, multiple subscribers:

```rust
// One publisher
struct Broadcaster {
    broadcast_pub: Hub<String>,
}

// Multiple subscribers automatically receive
struct Listener1 {
    broadcast_sub: Hub<String>,
}

struct Listener2 {
    broadcast_sub: Hub<String>,
}
```

### Many-to-One (Aggregation)

Multiple publishers, single subscriber:

```rust
// Multiple sensors publishing
struct Sensor1 {
    reading_pub: Hub<f32>,
}

struct Sensor2 {
    reading_pub: Hub<f32>,
}

// One aggregator subscribing
struct Aggregator {
    reading_sub: Hub<f32>,
}
```

### Processing Pipeline

Chain multiple nodes with pub/sub:

```rust
impl Node for ProcessorNode {
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        // Receive input
        if let Some(input) = self.input_sub.recv(&mut ctx) {
            // Process
            let output = self.process(input);

            // Publish result
            self.output_pub.send(output, &mut ctx).ok();
        }
    }
}
```

## Performance Characteristics

### Latency

| Message Size | Latency |
|--------------|---------|
| 16B (CmdVel) | 437ns |
| 304B (IMU) | 940ns |
| 1.5KB (LaserScan) | 2.2µs |
| 120KB (PointCloud) | 360µs |

**Latency scales linearly with message size.**

### Zero-Copy Design

Hub uses shared memory with atomic operations:
- No serialization/deserialization
- No network overhead
- No kernel context switches
- Cache-line aligned to prevent false sharing

## Best Practices

### Topic Naming

Use descriptive, consistent names:

```rust
// Good
Hub::new("cmd_vel")?;
Hub::new("sensor/lidar")?;
Hub::new("robot1/pose")?;

// Bad
Hub::new("data")?;  // Too vague
Hub::new("x")?;     // Not descriptive
```

### Error Handling

Always handle send errors:

```rust
if let Err(data) = hub.send(data, &mut ctx) {
    ctx.log_warning("Failed to publish message");
    // Handle error (retry, log, etc.)
}
```

### Type Safety

Use the same type for publishers and subscribers:

```rust
// Publisher
let pub_hub: Hub<f32> = Hub::new("velocity")?;

// Subscriber - must match type!
let sub_hub: Hub<f32> = Hub::new("velocity")?;
```

### Message Size

Prefer fixed-size types for predictable performance:

```rust
// Good - fixed size
Hub::<[f32; 100]>::new("data")?;

// Less ideal - dynamic allocation
Hub::<Vec<f32>>::new("data")?;
```

## Common Patterns

### Conditional Publishing

```rust
if value > threshold {
    hub.send(alert, &mut ctx).ok();
}
```

### Message Buffering

```rust
struct BufferedNode {
    data_sub: Hub<f32>,
    last_value: Option<f32>,
}

impl Node for BufferedNode {
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        // Update cache if new message available
        if let Some(value) = self.data_sub.recv(&mut ctx) {
            self.last_value = Some(value);
        }

        // Always have access to last value
        if let Some(value) = self.last_value {
            self.process(value);
        }
    }
}
```

### Rate Limiting

```rust
struct RateLimitedPublisher {
    data_pub: Hub<f32>,
    tick_count: u32,
}

impl Node for RateLimitedPublisher {
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        self.tick_count += 1;

        // Publish every 10 ticks (~6 Hz at 60 FPS)
        if self.tick_count % 10 == 0 {
            self.data_pub.send(42.0, &mut ctx).ok();
        }
    }
}
```

## Shared Memory Details

### Location

Hubs store data in `/dev/shm` (Linux shared memory):

```bash
# View HORUS shared memory
ls -lh /dev/shm/horus_*
```

### Cleanup

HORUS automatically cleans up on graceful shutdown (Ctrl+C).

Manual cleanup:
```bash
rm -f /dev/shm/horus_*
```

### Size Limits

Check available shared memory:
```bash
df -h /dev/shm
```

Typical: 50% of RAM or 32MB-2GB depending on system.

## See Also

- [Node API Reference](/api-node) - Node implementation
- [Scheduler API Reference](/api-scheduler) - Node orchestration
- [Core Concepts: Hub](/core-concepts-hub) - Detailed guide
- [Message Types](/message-types) - Standard messages
