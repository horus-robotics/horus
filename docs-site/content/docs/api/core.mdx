...
Creates a scheduler with default settings (60 Hz tick rate).
...
```rust
pub fn new_realtime() -> HorusResult<Self>
```

Creates a hard real-time scheduler with deterministic execution.

**Returns:** `HorusResult<Scheduler>`
...
Creates a deterministic scheduler for simulation and testing.
...
Pre-allocates capacity for nodes.
...
**Available Presets:**
- `SchedulerConfig::standard()` - Balanced performance
- `SchedulerConfig::hard_realtime()` - Deterministic, &lt;20μs jitter
...
Enables deterministic execution mode.
...
|-----------|---------|-------|
| `Hub::send()` | ~200ns | Zero-copy shared memory |
| `Hub::recv()` | ~85-167ns | Non-blocking |
| `Link::send()` | ~50ns | Lock-free SPSC |
| `Link::recv()` | ~30ns | Lock-free SPSC |
