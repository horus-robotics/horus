...
- `send(msg, ctx)` - Publish a message (437ns latency)
...
// Subscribe
if let Some(value) = hub.recv(&mut ctx) {
...
impl Node for TemperatureSensor {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        self.temperature_pub.send(temp, &mut ctx).ok();
    }
}
...
Execution priority values (lower = higher priority):

```rust
// Recommended priority constants
const CRITICAL: u32    = 0;   // Highest
const HIGH: u32        = 10;
const NORMAL: u32      = 50;  // Default
const LOW: u32         = 80;
const BACKGROUND: u32  = 100; // Lowest
```
...
fn init(&mut self, ctx: &mut NodeInfo) -> Result<()> {
    self.device.connect()
        .map_err(|e| HorusError::InitializationFailed(format!("Connection failed: {}", e)))?;
    Ok(())
}
```
...
fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
    if let Some(data) = self.sub.recv(&mut ctx) {
        self.pub.send(data * 2.0, &mut ctx).ok();
    }
}
...
hub.send(data, &mut ctx);  // .ok() or .unwrap() without handling
```
...
- **Tools**: [CLI Reference](/development/cli-reference)
- **Performance**: [Performance Guide](/performance/performance)
