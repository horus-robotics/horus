...
**IMPORTANT: Debug builds are 20-100x slower than release builds!**

**Debug mode** (default): Fast to compile, slow to run
- Good for: Quick iteration during development
- Tick time: 60-200μs (way too slow for production!)
- Adds overflow checks, bounds checking, assertions, no optimizations

**Release mode** (`--release`): Slow to compile, fast to run
- Good for: Performance testing, benchmarks, production, real robot testing
- Tick time: 1-3μs (20-100x faster!)
