...
The `horus` command gives you everything you need to build, run, and manage your applications. This page covers all 9 commands.
...
- Speed: Messages take ~248ns-1.31μs (100x faster!)
...
$ horus check
✓ HORUS framework installed (v0.1.0)
✗ Missing dependency: horus-vision (required by horus.yaml)
✗ Shared memory not writable (/dev/shm)

⚠ Fix these issues and run 'horus check' again
```
...
Options:
  -s, --scenario <NAME>    Load predefined scenario (warehouse, maze, etc.)
  --headless               Run without GUI (for CI/CD)
  -p, --port <PORT>        Web UI port (default: 3001)
  -r, --realtime           Lock to real-time physics (default: as-fast-as-possible)
```
...
- **Sensor simulation** - Virtual LiDAR, cameras, IMU
- **Visual debugging** - See robot state and sensor readings
- **Web interface** - Control simulation from browser
...
**Custom scenario**:
```bash
# Create scenario file: warehouse.yaml
# Then load it:
horus sim --scenario warehouse
```
...
**Simulator vs Reality**: The same HORUS topics (`cmd_vel`, `laser_scan`, etc.) work in both. No code changes needed!

See [Simulation Guide](/simulation) for advanced features, custom scenarios, and RL integration.
