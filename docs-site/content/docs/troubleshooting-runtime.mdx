...
1. **Check the Dashboard**: Run `horus dashboard` to see active nodes, topics, and message flow
   ...
   **Fix**: Check multiple ticks:
   ...
   impl Node for SubscriberNode {
       fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
           if let Some(msg) = self.hub.recv(ctx) {
               // Process message
           } else {
...
```rust
use horus::prelude::*;
use horus_library::messages::*;  // For CmdVel, LaserScan, etc.
```
...
The HORUS dashboard is your best debugging tool. It shows real-time information about your running application.
...
fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
    if let Some(ctx) = ctx {
        ...
        ctx.log_warn("Something unusual happened");
...
```rust
fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
    let msg = self.hub.recv(ctx).unwrap();  // PANIC if no message!
}
```

### ✅ DO: Use Result for errors

```rust
impl Node for MyNode {
    fn init(&mut self, ctx: &mut NodeInfo) -> Result<(), String> {
        if self.sensor.is_broken() {
            return Err("Sensor initialization failed".to_string());
        }
        Ok(())
    }
}
...
- Review [Core Concepts - Nodes](/core-concepts-nodes) for node lifecycle details
- Learn about [Testing](/testing) to catch errors early
- Explore [Examples](/examples) for working code patterns
- Read [Performance](/performance) to optimize your application
