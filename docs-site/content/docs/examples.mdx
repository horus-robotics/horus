...
scheduler.register(horus.Node("sensor", pubs="imu_data", tick=high_freq_sensor),
                   priority=0, rate_hz=100.0)
scheduler.register(horus.Node("control", subs="imu_data", pubs="cmd_vel", tick=control_loop),
                   priority=1, rate_hz=50.0)
scheduler.register(horus.Node("log", subs="cmd_vel", tick=logger),
...
fn main() -> HorusResult<()> {
    let mut scheduler = Scheduler::new();
    scheduler.register(Box::new(TempSensor {
