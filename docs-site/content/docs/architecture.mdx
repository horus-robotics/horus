...
- **horus_backend.rs**: Shared memory IPC with serde serialization (312ns-481ns)
...
**Performance:**
- CmdVel (16B): **481ns** (Hub) / **312ns** (Link)
- IMU (304B): **940ns** (Hub)
...
|-----------|---------|----------|
| **Link** (SPSC) | 312ns | Ultra-low latency point-to-point |
| **Hub** (MPMC) | 481ns | General purpose pub/sub |
...
**Implications:**
- For 60 Hz control loops: ✓ Works perfectly
- For 100 Hz+ control loops: ⚠️ Currently not achievable (coming in future release)
...
**Roadmap:** Per-node rate control and configurable tick rates are planned for post-beta releases. See [Roadmap](/roadmap) for details.
...
── SensorNode publish:     481ns
── Shared memory write:    180ns
── ControlNode receive:    481ns
── Processing:            1000ns (user code)
── ControlNode publish:    481ns
── MotorNode receive:      481ns
```
...
|--------|--------------|-------------|------------|-------------|
| **CmdVel latency** | 312ns | 481ns | 50-100μs | **104-320x faster** |
