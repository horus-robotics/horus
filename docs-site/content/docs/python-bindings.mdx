...
scheduler.register(sensor_node, priority=0, logging=True, rate_hz=100.0)

# Medium-frequency controller (50Hz)
scheduler.register(control_node, priority=1, logging=False, rate_hz=50.0)

# Low-frequency logger (10Hz)
scheduler.register(logger_node, priority=2, logging=True, rate_hz=10.0)
...
```python
scheduler.register(sensor, priority=0)     # Executes FIRST
scheduler.register(controller, priority=1) # Executes SECOND
scheduler.register(actuator, priority=2)   # Executes THIRD
```
...
# High-frequency sensor (100Hz)
scheduler.register(sensor, priority=0, rate_hz=100.0)

# Medium-frequency control (50Hz)
scheduler.register(controller, priority=1, rate_hz=50.0)

# Low-frequency logging (10Hz)
scheduler.register(logger, priority=2, rate_hz=10.0)
...
scheduler = horus.Scheduler()
scheduler.register(my_node, priority=0, logging=True)  # Enable logs
