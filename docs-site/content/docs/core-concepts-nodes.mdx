...
fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
    if let Some(ctx) = ctx {
        let avg_duration = ctx.metrics.avg_tick_duration_us;
...
- Learn about [Hub and Pub/Sub](/docs/core-concepts-hub) for inter-node communication
- Understand the [Scheduler](/docs/core-concepts-scheduler) for orchestrating nodes
- Explore [Message Types](/docs/message-types) for standard robotics data
- Read the [API Reference](/docs/api-node) for complete Node trait documentation
