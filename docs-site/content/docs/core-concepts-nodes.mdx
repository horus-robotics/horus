...
- When to use different NodePriority levels (Critical for safety, Background for logging)
...
// Output: [12:34:56.789] [IPC: 481ns | Tick: 12μs] MyNode --PUB--> 'cmd_vel' = 1.5
```
...
scheduler.add_with_priority(safety_node, NodePriority::Critical);

// Controller runs second
scheduler.add_with_priority(control_node, NodePriority::High);

// Sensors run third
scheduler.add_with_priority(sensor_node, NodePriority::Normal);

// Logging runs LAST
scheduler.add_with_priority(logger_node, NodePriority::Background);
```
