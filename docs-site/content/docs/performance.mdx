...
HORUS achieves **366ns-2.8µs latency** and is **100-270x faster than ROS2**. This guide shows you how to maximize performance.
...
| Message Type | Size | HORUS | ROS2 DDS | Speedup |
|--------------|------|-------|----------|---------|
| CmdVel | 16B | 366ns | 50-100µs | **270x** |
| IMU | 304B | 543ns | 80-150µs | **180x** |
| LaserScan | 1.5KB | 1.58µs | 150-300µs | **150x** |
...
**Gains**: 5-15% from CPU-specific SIMD instructions.
...
scheduler.set_tick_rate(1000);  // 1000 FPS = 1ms per tick

// For 10kHz control
scheduler.set_tick_rate(10000);  // 100µs per tick
```

**Higher FPS = more overhead**. Match your control requirements.
...
- `avg_tick_duration_us`: Average tick time
- `max_tick_duration_us`: Worst-case tick time
- `total_messages_sent`: Messages published
...
```
[12:34:56.789] [IPC: 366ns | Tick: 12µs] PublisherNode --PUB--> 'cmd_vel' = 1.5
```
...
- Apply these optimizations to your [Examples](/docs/examples)
- Learn about [Multi-Language Support](/docs/multi-language)
- Read the [Core Concepts](/docs/core-concepts-nodes) for deeper understanding
- Check the [CLI Reference](/docs/cli-reference) for build options
