...
That's **100-270x faster** than traditional approaches.
...
**C++** - Integrate existing hardware drivers (Alpha):
```cpp
#include "horus.hpp"

horus::System system("sensor");
horus::Publisher<float> temp_pub("temperature");

temp_pub.publish(25.0);
```

**Rust** - Full framework power for performance-critical code.

Mix and match languages in the same application!
...
- Mix Rust (performance) + Python (ease) + C (drivers)
- Integrate legacy C code with modern Rust
...
**Over-the-internet communication** - HORUS uses shared memory:
- For network communication, use gRPC, HTTP, WebSockets
- For distributed systems across machines, use message queues (RabbitMQ, Kafka)
...
**Tiny embedded systems** - Microcontrollers without Linux:
- HORUS requires `/dev/shm` (Linux shared memory)
...
struct SensorNode { pub: Hub<f32> }

// ProcessNode - can swap implementation
struct ProcessNode { sub: Hub<f32>, pub: Hub<f32> }

// DisplayNode - can replace with LogNode, etc.
struct DisplayNode { sub: Hub<f32> }
```
...
**HORUS:**
- 0.312-0.481μs latency (sub-microsecond)
...
HORUS uses Linux's `/dev/shm` for inter-process communication:

- **Location:** `/dev/shm/horus/topics/`
...
3. [First Application](/getting-started/first-application) - Detailed walkthrough

**Core concepts:**
4. [Nodes](/core-concepts-nodes) - Build components
5. [Hub](/core-concepts-hub) - Pub/sub communication
6. [Scheduler](/core-concepts-scheduler) - Run your application

**Practical features:**
7. [node! Macro](/node-macro) - Reduce boilerplate
8. [message! Macro](/message-macro) - Custom message types
9. [Dashboard](/dashboard) - Monitor and debug

**Advanced:**
10. [Multi-Language](/multi-language) - Python and C integration
11. [Performance](/performance) - Optimize for speed
12. [Examples](/examples) - Real projects
...
**Questions?** Check out [CLI Reference](/cli-reference) or [Architecture](/architecture).
