...
impl Node for FilterNode {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        ...
        if let Some(raw_temp) = self.raw_sub.recv(ctx) {
            ...
            self.filtered_pub.send(filtered, ctx).ok();
...
impl Node for DisplayNode {
    ...
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        if let Some(temp) = self.filtered_sub.recv(ctx) {
...
### Metrics Tab
- **IPC Latency**: 312ns-481ns (sub-microsecond!)
...
**Fix:**
- Ensure HORUS is installed: `horus --version`
...
```rust
if temp > 30.0 {
    println!("⚠️  WARNING: High temperature detected!");
}
```
...
1. **[Testing](/testing)** - Learn how to unit test your nodes
2. **[Using Pre-Built Nodes](/using-prebuilt-nodes)** - Use library nodes instead of writing from scratch
3. **[node! Macro](/node-macro)** - Reduce boilerplate with macros
4. **[Message Types](/message-types)** - Use complex message types instead of primitives
