...
HORUS achieves **312ns-2.8µs latency** through a zero-copy shared memory IPC architecture. This page explains how HORUS uses `/dev/shm` for ultra-low latency inter-process communication.
...
- Read the [Performance Guide](/performance) for optimization tips
