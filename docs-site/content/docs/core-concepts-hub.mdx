...
- How Hub provides zero-copy, lock-free pub/sub communication with sub-microsecond latency (312ns-481ns)
...
The Hub is HORUS's ultra-low latency publish-subscribe (pub/sub) communication system. It enables nodes to exchange messages through shared memory IPC with **312ns-481ns latency** (Link/Hub).
...
**Sub-Microsecond Latency**: 312ns (Link) / 481ns (Hub) for small messages (16B)
