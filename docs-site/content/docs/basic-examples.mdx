...
impl Node for SensorNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        ctx.log_debug(&format!("Published: {:.2}", reading));
...
> [WARNING]️ **IMPORTANT - Multi-Process Communication**
>
...
impl Node for ProcessorNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        if let Some(data) = self.data_sub.recv(ctx) {
            let processed = data * 2.0;
            ctx.log_info(&format!("Received: {:.2} -> {:.2}", data, processed));
        }
    }
...
impl Node for TeleopNode {
    ...
    fn shutdown(&mut self, ctx: &mut NodeInfo) -> Result<()> {
        ...
        self.cmd_pub.send(stop, None).ok();
        ...
        if let Some(scan) = self.scan_sub.recv(ctx) {
            ...
            if let Some(min_dist) = scan.min_range() {
                if min_dist < self.safety_distance {
                    ...
                    // Safe to move
                    ctx.log_debug(&format!("Safe - closest obstacle: {:.2}m", min_dist));
                }
            }
        }
    }
...
impl Node for TestSystem {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        if let Some(output) = self.output_sub.recv(ctx) {
            self.position += output * 0.01;  // Simple integration
        }

        // Publish current position
        self.feedback_pub.send(self.position, ctx).ok();
    }
...
impl Node for SetpointNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        // Target position
        let setpoint = 10.0;
        self.setpoint_pub.send(setpoint, ctx).ok();
    }
...
impl Node for SensorNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        ctx.log_debug(&format!("Raw: {:.2}", raw_data));
...
impl Node for FilterNode {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        if let Some(raw) = self.raw_sub.recv(ctx) {
            ...
            ctx.log_debug(&format!("Filtered: {:.2}", self.filtered_value));
        }
    }
...
- [Second Application Tutorial](/second-application) - Build a 3-node sensor pipeline
- [Advanced Examples](/advanced-examples) - State machines, multi-process systems, Python integration
- [Testing](/testing) - Write tests for your nodes
- [Using Prebuilt Nodes](/using-prebuilt-nodes) - Leverage the standard library
- [Message Types](/message-types) - Complete message reference

Stuck? Check:
- [Troubleshooting Runtime Errors](/troubleshooting-runtime) - Fix common issues
- [Dashboard](/dashboard) - Debug with the web UI
