...
scheduler.add(Box::new(lidar), 0, Some(true));
```
...
scheduler.add(Box::new(drive), 10, Some(true));
```
...
scheduler.add(Box::new(planner), 5, Some(true));
```
...
scheduler.add(Box::new(localization), 5, Some(true));
```
...
scheduler.add(Box::new(modbus), 10, Some(true));
```
...
scheduler.add(Box::new(servos), 10, Some(true));
```
...
scheduler.add(Box::new(encoder), 0, Some(true));
```
...
```toml
[dependencies]
horus_library = { version = "0.1.4", features = ["full"] }

# Or specific features:
horus_library = { version = "0.1.4", features = [
