...
> **⚠️ Under Development**: The parameters system is currently under active development. The core parameter API (`ctx.params.get_*()` and `ctx.params.set()`) works as documented, but the dashboard UI for parameter management and some advanced features are still being finalized. Check back for updates.
...
See [Dashboard Guide](/dashboard#parameter-management) for API details.
...
- **[Dashboard Guide](/dashboard)** - Manage parameters via web interface
- **[node! Macro Guide](/node-macro)** - Learn about `init()` and `tick()` hooks
- **[Examples](/examples)** - See real-world parameter usage
- **[CLI Reference](/cli-reference)** - Command-line parameter tools
