...
impl Node for TemperatureMonitor {
    ...
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        ...
        if let Some(temp) = self.subscriber.recv(ctx) {
            ctx.log_info(&format!("Temperature: {:.1}°C", temp));
        }
    }
}
...
> 💡 **See [Configuration](/configuration#session-management)** for details on session_id and [Shared Memory](/core-concepts-shared-memory#session-management) for how sessions work.
...
impl Node for TemperatureMonitor {
    fn tick(&mut self, mut ctx: Option<&mut NodeInfo>) {
        if let Some(temp) = self.subscriber.recv(ctx) {
            ...
            // Alert if too hot!
            if temp > 25.0 {
                ctx.log_warning("Temperature too high!");
            }
        }
    }
}
