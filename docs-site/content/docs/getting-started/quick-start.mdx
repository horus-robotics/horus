...
impl Node for TemperatureMonitor {
    fn tick(&mut self, ctx: Option<&mut NodeInfo>) {
        if let Some(temp) = self.subscriber.recv(ctx) {
            ...
            // Alert if too hot!
            if temp > 25.0 {
                println!("⚠️  WARNING: Temperature too high!");
            }
        }
    }
}
...
**Core Concepts:**
- **[Nodes](/docs/core-concepts-nodes)** - Deep dive into the Node pattern
- **[Hub](/docs/core-concepts-hub)** - How ultra-fast communication works
- **[Scheduler](/docs/core-concepts-scheduler)** - Priority-based execution

**Make Development Easier:**
- **[node! Macro](/docs/node-macro)** - Eliminate boilerplate code
- **[CLI Reference](/docs/cli-reference)** - All the `horus` commands
- **[Dashboard](/docs/dashboard)** - Monitor your application visually

**See More Examples:**
- **[Examples](/docs/examples)** - Real applications you can run
- **[Multi-Language](/docs/multi-language)** - Use Python or C instead
...
1. **[Use the node! macro](/docs/node-macro)** to eliminate boilerplate
2. **[Run the examples](/docs/examples)** to see real applications
3. **[Open the dashboard](/docs/dashboard)** to monitor your system
