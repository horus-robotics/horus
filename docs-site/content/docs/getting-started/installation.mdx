...
### "linker `cc` not found" or "pkg-config not found"

You need build tools and development libraries:
...
### Build fails with "`libudev-sys`", "`openssl-sys`", or "`alsa-sys`"

You need system development libraries:

```bash
# Ubuntu/Debian - Complete fix
sudo apt install libudev-dev libssl-dev libasound2-dev

# Fedora/RHEL
sudo dnf install systemd-devel openssl-devel alsa-lib-devel

# Arch Linux
sudo pacman -S systemd openssl alsa-lib
