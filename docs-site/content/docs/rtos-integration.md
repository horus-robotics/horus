...
HORUS abstracts the underlying RTOS through a common interface, supporting:

- **RT-Linux (PREEMPT_RT)** - Linux with real-time patches
...
```toml
[dependencies]
horus_core = { version = "0.1.4", features = ["rtos", "rt-linux"] }
```

Available features:
- `rtos` - Core RTOS abstraction
- `rt-linux` - RT-Linux backend
- `freertos` - FreeRTOS backend
- `zephyr` - Zephyr backend
- `qnx` - QNX backend
- `bare-metal` - Bare metal support
