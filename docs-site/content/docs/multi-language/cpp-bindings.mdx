...
> -  Limited to built-in message types only
...
**Rust control node**:
```rust
let cmd: Twist = hub.recv(&mut ctx);
...
For full flexibility, use [Rust API](/api/api) or [Python bindings](/multi-language/python-bindings).
...
- Read [Multi-Language](/multi-language/multi-language) for cross-language communication
