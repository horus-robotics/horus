...
impl PyScheduler {
    ...
    fn set_tick_rate(&mut self, rate_hz: f64) -> PyResult<()> {
        if rate_hz <= 0.0 || rate_hz > 10000.0 {
            return Err(PyRuntimeError::new_err("Tick rate must be between 0 and 10000 Hz"));
        }
    ...
    fn run_for(&mut self, py: Python, duration_seconds: f64) -> PyResult<()> {
        ...
        for tick in 0..total_ticks {
            ...
            if elapsed < tick_duration {
                ...
                eprintln!("Warning: Tick {} took {:?}, longer than period {:?}", 
    ...
    fn __repr__(&self) -> PyResult<String> {
        let nodes = self.nodes.lock()
            .map_err(|e| PyRuntimeError::new_err(format!("Failed to lock nodes: {}", e)))?;
        Ok(format!("Scheduler(nodes={}, tick_rate={}Hz)", nodes.len(), self.tick_rate_hz))
    }
}