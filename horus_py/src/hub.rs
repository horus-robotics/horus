...
impl PyHub {
    ...
    fn send(&self, py: Python, message: PyObject) -> PyResult<bool> {
        ...
        let data = if let Ok(bytes) = message.extract::<Vec<u8>>(py) {
            ...
            let json_str = serde_json::to_string(&pythonize::depythonize::<serde_json::Value>(dict)?)
                .map_err(|e| PyRuntimeError::new_err(format!("Failed to serialize dict: {}", e)))?;
            json_str.into_bytes()
        } else {
            ...
            if let Some(metadata) = &msg.metadata {
                if metadata != &self.hub_type {
                    return Err(PyRuntimeError::new_err(
                        format!("Type mismatch: expected '{}', got '{}'", self.hub_type, metadata)
                    ));
                }
            }
