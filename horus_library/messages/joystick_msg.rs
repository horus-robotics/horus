...
impl JoystickInput {
    ...
    pub fn new_connection(joystick_id: u32, connected: bool) -> Self {
        ...
        let mut element_name = [0u8; 32];
        let name_str = if connected { "Connected" } else { "Disconnected" };
    ...
    pub fn get_event_type(&self) -> String {
        let end = self.event_type.iter().position(|&b| b == 0).unwrap_or(self.event_type.len());
        String::from_utf8_lossy(&self.event_type[..end]).into_owned().to_string()
    }
    
    /// Get the element name as a String
    pub fn get_element_name(&self) -> String {
        let end = self.element_name.iter().position(|&b| b == 0).unwrap_or(self.element_name.len());
        String::from_utf8_lossy(&self.element_name[..end]).into_owned().to_string()
    }
