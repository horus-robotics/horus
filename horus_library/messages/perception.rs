...
mod tests {
    ...
    #[test]
    fn test_point_field() {
        let field = PointField::new("x", 0, PointFieldType::Float32, 1);
        assert_eq!(field.name_str(), "x");
        assert_eq!(field.field_size(), 4);

        let rgb_field = PointField::new("rgb", 12, PointFieldType::UInt32, 1);
        assert_eq!(rgb_field.offset, 12);
        assert_eq!(rgb_field.field_size(), 4);
    }
    ...
    fn test_point_cloud_xyz() {
        ...
        assert_eq!(cloud.field_count, 3);
        assert_eq!(cloud.point_step, 12);
        assert!(cloud.is_valid());

        // Test extraction
        let extracted = cloud.extract_xyz().unwrap();
        assert_eq!(extracted.len(), 2);
        assert!((extracted[0].x - 1.0).abs() < 1e-6);
        assert!((extracted[1].z - 6.0).abs() < 1e-6);
    }
    ...
    fn test_bounding_box_3d() {
        ...
        let bbox = BoundingBox3D::new(center, size).with_label("car");

        assert_eq!(bbox.label_str(), "car");
        ...
        assert!(corners.iter().all(|p| bbox.contains_point(p)));
    }

    #[test]
    fn test_depth_image() {
        let depths = vec![1000, 2000, 0, 1500]; // 1m, 2m, invalid, 1.5m
        let mut depth_img = DepthImage::new(2, 2, depths);

        assert_eq!(depth_img.get_depth(0, 0), Some(1000));
        assert_eq!(depth_img.get_depth(1, 1), Some(1500));
        assert!(!depth_img.is_valid_depth(0));
        assert!(depth_img.is_valid_depth(1000));
    ...
    fn test_plane_detection() {
        // Horizontal plane at z=1 (normal pointing up)
        let coeffs = [0.0, 0.0, 1.0, -1.0]; // z - 1 = 0
        let center = Point3::new(0.0, 0.0, 1.0);
        let normal = Vector3::new(0.0, 0.0, 1.0);

        let plane = PlaneDetection::new(coeffs, center, normal).with_type("floor");

        // Point on the plane
        let on_plane = Point3::new(5.0, 5.0, 1.0);
        assert!(plane.contains_point(&on_plane, 0.1));

        // Point above the plane
        let above_plane = Point3::new(0.0, 0.0, 2.0);
        assert!((plane.distance_to_point(&above_plane) - 1.0).abs() < 1e-10);
        assert!(!plane.contains_point(&above_plane, 0.1));

        assert_eq!(plane.plane_type_str(), "floor");
    }

    #[test]
    fn test_bounding_box_array() {
        let mut array = BoundingBoxArray3D::new();

        let bbox1 = BoundingBox3D::new(Point3::new(1.0, 0.0, 0.0), Vector3::new(1.0, 1.0, 1.0));
        let bbox2 = BoundingBox3D::new(Point3::new(2.0, 0.0, 0.0), Vector3::new(2.0, 2.0, 2.0));

        array.add_box(bbox1).unwrap();
        array.add_box(bbox2).unwrap();

        assert_eq!(array.count, 2);
        assert_eq!(array.get_boxes().len(), 2);

        let filtered = array.filter_by_confidence(0.5);
        assert_eq!(filtered.len(), 2); // Both have default confidence 1.0
    }
}
