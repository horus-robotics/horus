...
impl AtomicHubMetrics {
    ...
    pub fn snapshot(&self) -> HubMetrics {
        HubMetrics {
            ...
            send_failures: self.send_failures.load(std::sync::atomic::Ordering::Relaxed),
            recv_failures: self.recv_failures.load(std::sync::atomic::Ordering::Relaxed),
...
impl<T> Clone for Hub<T> {
    fn clone(&self) -> Self {
        Self {
            ...
            state: std::sync::atomic::AtomicU8::new(self.state.load(std::sync::atomic::Ordering::Relaxed)),
...
impl<T: Send + Sync + 'static + Clone + std::fmt::Debug> Hub<T> {
    ...
    pub fn new_with_capacity(topic_name: &str, capacity: usize) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        Ok(Hub {
            ...
            state: std::sync::atomic::AtomicU8::new(ConnectionState::Connected.to_u8()),
        ...
        T: std::fmt::Debug + Clone
    {
        ...
        let ipc_ns = match self.shm_topic.loan() {
            Ok(mut sample) => {
                ...
                self.state.store(ConnectionState::Connected.to_u8(), std::sync::atomic::Ordering::Relaxed);

                ipc_time
            },
            Err(_) => {
                // Lock-free atomic increment for failure metrics (OPTIMIZED)
                self.metrics.send_failures.fetch_add(1, std::sync::atomic::Ordering::Relaxed);

                // Lock-free state update (OPTIMIZED)
                self.state.store(ConnectionState::Failed.to_u8(), std::sync::atomic::Ordering::Relaxed);
        ...
        T: std::fmt::Debug
    {
        ...
        match result {
            ...
            None => {
                ...
                self.metrics.recv_failures.fetch_add(1, std::sync::atomic::Ordering::Relaxed);
                None
            }
        }
    }
