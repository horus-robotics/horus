    ...
    T: Send + Sync + 'static,
{
    ...
    pub fn new(service_name: &str) -> Result<Self, IceoryxError> {
        ...
        let service = Arc::new(node)
            ...
            .create()
            .map_err(|e| {
                IceoryxError::ServiceCreation(format!("Service creation failed: {:?}", e))
            })?;
    ...
    pub fn create_publisher(&self) -> Result<super::Publisher<T>, IceoryxError> {
        let publisher = self.service.publisher_builder().create().map_err(|e| {
            IceoryxError::PublisherCreation(format!("Publisher creation failed: {:?}", e))
        })?;
    ...
    pub fn create_subscriber(&self) -> Result<super::Subscriber<T>, IceoryxError> {
        let subscriber = self.service.subscriber_builder().create().map_err(|e| {
            IceoryxError::SubscriberCreation(format!("Subscriber creation failed: {:?}", e))
        })?;
