...
mod tests {
    ...
    fn test_link_producer_consumer() {
        ...
        // Producer sends - single-slot design overwrites
        assert!(producer.send(42, None).is_ok());
        assert!(producer.send(43, None).is_ok());

        // Consumer receives only the latest (43)
    ...
    #[test]
    fn test_link_single_slot_overwrite() {
        // Clean up before test
        let _ = std::fs::remove_file("/dev/shm/horus/topics/horus_links_test_single_slot");

        let producer = Link::<i32>::producer("test_single_slot").unwrap();
        let consumer = Link::<i32>::consumer("test_single_slot").unwrap();

        // Send multiple messages - they overwrite each other
        assert!(producer.send(1, None).is_ok());
        assert!(producer.send(2, None).is_ok());
        assert!(producer.send(3, None).is_ok());

        // Consumer should only see the latest value (3)
        assert_eq!(consumer.recv(None), Some(3));
    ...
    fn test_link_ipc() {
        ...
        // Consumer should receive only the latest value (43) - single-slot design
    ...
    fn test_link_metrics() {
        // Test metrics tracking with local memory (each Link tracks its own metrics)
        ...
        // Send 10 messages (they overwrite each other in single-slot)
        for i in 0..10 {
            producer.send(i, None).unwrap();
        }

        // Check producer metrics - producer tracks what it sent
        let metrics = producer.get_metrics();
        assert_eq!(metrics.messages_sent, 10);
        assert_eq!(metrics.messages_received, 0); // Producer doesn't receive
        assert_eq!(metrics.send_failures, 0); // Single-slot never fails

        // Receive once (gets latest value)
        consumer.recv(None);

        // Check consumer metrics - consumer tracks what it received
        let metrics = consumer.get_metrics();
        assert_eq!(metrics.messages_sent, 0); // Consumer doesn't send
        assert_eq!(metrics.messages_received, 1);
        assert_eq!(metrics.send_failures, 0);

        // Single-slot design never fails - no buffer full condition
        for i in 0..1000 {
            assert!(producer.send(i, None).is_ok());
        }

        // Should still be zero failures
        let metrics = producer.get_metrics();
        assert_eq!(metrics.send_failures, 0);
