...
mod tests {
    ...
    // Test node that simulates I/O operation
    struct IONode {
        name: &'static str,
    ...
    #[tokio::test]
    async fn test_async_io_executor() {
        let mut executor = AsyncIOExecutor::new().unwrap();
        let counter = Arc::new(AtomicUsize::new(0));

        let node = Box::new(IONode {
            name: "TestIONode",
        ...
        executor.spawn_node(node, None, tx).unwrap();
    ...
    async fn test_multiple_async_nodes() {
        ...
        let node1 = Box::new(IONode {
            name: "IONode1",
            counter: Arc::clone(&counter1),
            delay_ms: 10,
        });

        let node2 = Box::new(IONode {
            name: "IONode2",
        ...
        executor.spawn_node(node1, None, tx.clone()).unwrap();
        executor.spawn_node(node2, None, tx).unwrap();

        let start = Instant::now();

        // Trigger tick for all nodes
        executor.tick_all().await;
        ...
        // Total time should be ~20ms (max delay) not 30ms (sum)
