...
mod tests {
    ...
    fn test_jit_arithmetic() {
        ...
        // Compile: output = input * 3 + 7
        let func_ptr = compiler
            .compile_arithmetic_node("test_arithmetic", 3, 7)
            .unwrap();

        unsafe {
            // Test: 10 * 3 + 7 = 37
            let result = compiler.execute_arithmetic(func_ptr, 10);
            assert_eq!(result, 37);

            // Test: 5 * 3 + 7 = 22
            let result = compiler.execute_arithmetic(func_ptr, 5);
            assert_eq!(result, 22);
        }
    }

    #[test]
    fn test_jit_dataflow() {
        let mut compiler = JITCompiler::new().unwrap();

        // Compile: output = (a + b) * (c - d)
        let func_ptr = compiler.compile_dataflow_combiner("test_combiner").unwrap();

        unsafe {
            // Test: (2 + 3) * (10 - 4) = 5 * 6 = 30
            let result = compiler.execute_combiner(func_ptr, 2, 3, 10, 4);
            assert_eq!(result, 30);

            // Test: (7 + 8) * (20 - 5) = 15 * 15 = 225
            let result = compiler.execute_combiner(func_ptr, 7, 8, 20, 5);
