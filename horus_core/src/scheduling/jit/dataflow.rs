...
mod tests {
    ...
    /// Test node that does simple arithmetic
    struct TestArithmeticNode {
        counter: i64,
    }

    impl Node for TestArithmeticNode {
        fn name(&self) -> &'static str {
            "test_arithmetic"
        }
    ...
    impl DataflowNode for TestArithmeticNode {
        fn get_dataflow_expr(&self) -> Option<DataflowExpr> {
            // Express: output = input * 2 + 1
            Some(DataflowExpr::BinOp {
                op: BinaryOp::Add,
                left: Box::new(DataflowExpr::BinOp {
                    op: BinaryOp::Mul,
                    left: Box::new(DataflowExpr::Input("counter".to_string())),
                    right: Box::new(DataflowExpr::Const(2)),
                }),
                right: Box::new(DataflowExpr::Const(1)),
            })
        }
    }

    #[test]
    fn test_dataflow_compilation() {
        let expr = DataflowExpr::BinOp {
            op: BinaryOp::Add,
            left: Box::new(DataflowExpr::BinOp {
                op: BinaryOp::Mul,
                left: Box::new(DataflowExpr::Input("x".to_string())),
                right: Box::new(DataflowExpr::Const(3)),
            }),
            right: Box::new(DataflowExpr::Const(5)),
        };

        let mut compiled = CompiledDataflow::compile("test".to_string(), expr).unwrap();

        // Execute multiple times
        for i in 0..100 {
            let result = compiled.execute(i);
            // Our test compiler does input * 2 + 1
            assert_eq!(result, i * 2 + 1);
        }

        // Check performance
        println!("Average execution time: {:.2} ns", compiled.avg_exec_ns());
        assert!(compiled.is_fast_enough());
    }
}
