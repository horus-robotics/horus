...
mod tests {
    ...
    fn test_simple_chain() {
        ...
        let nodes = vec![
            ("A", vec!["topic1".to_string()], vec![]),
            ("B", vec!["topic2".to_string()], vec!["topic1".to_string()]),
            ("C", vec![], vec!["topic2".to_string()]),
        ];
        ...
        assert_eq!(graph.levels.len(), 3); // 3 sequential levels
    ...
    fn test_parallel_nodes() {
        ...
        assert_eq!(graph.nodes.len(), 3);
        assert_eq!(graph.edges.len(), 2);
        assert_eq!(graph.levels.len(), 2); // Level 0: A,B | Level 1: C
        assert_eq!(graph.levels[0].len(), 2); // A and B can run in parallel
        assert!(!graph.has_cycles());
    }
    ...
    fn test_diamond_pattern() {
        ...
        let nodes = vec![
            ("A", vec!["topic1".to_string()], vec![]),
            ("B", vec!["topic2".to_string()], vec!["topic1".to_string()]),
            ("C", vec!["topic3".to_string()], vec!["topic1".to_string()]),
            (
                "D",
                vec![],
        ...
        assert_eq!(graph.levels.len(), 3); // Level 0: A | Level 1: B,C | Level 2: D
        assert_eq!(graph.levels[1].len(), 2); // B and C can run in parallel
        assert!(!graph.has_cycles());
    }
    ...
    fn test_independent_nodes() {
        ...
        let nodes = vec![
            ("A", vec!["topic1".to_string()], vec![]),
            ("B", vec!["topic2".to_string()], vec![]),
            ("C", vec!["topic3".to_string()], vec![]),
        ];

        let graph = DependencyGraph::from_nodes(&nodes);

        assert_eq!(graph.nodes.len(), 3);
        assert_eq!(graph.edges.len(), 0); // No dependencies
        assert_eq!(graph.levels.len(), 1); // All in same level
        assert_eq!(graph.levels[0].len(), 3); // All can run in parallel
        assert!(!graph.has_cycles());
    }

    #[test]
    fn test_complex_graph() {
        // Sensor -> [ProcessA, ProcessB] -> Fusion -> Control
        let nodes = vec![
            ("Sensor", vec!["raw_data".to_string()], vec![]),
            (
                "ProcessA",
                vec!["processed_a".to_string()],
                vec!["raw_data".to_string()],
            ),
            (
                "ProcessB",
                vec!["processed_b".to_string()],
                vec!["raw_data".to_string()],
            ),
            (
                "Fusion",
                vec!["fused".to_string()],
                vec!["processed_a".to_string(), "processed_b".to_string()],
            ),
            ("Control", vec![], vec!["fused".to_string()]),
        ];

        let graph = DependencyGraph::from_nodes(&nodes);

        assert_eq!(graph.nodes.len(), 5);
        assert_eq!(graph.levels.len(), 4);
        assert_eq!(graph.levels[1].len(), 2); // ProcessA and ProcessB in parallel
