...
impl<T> ShmTopic<T> {
    ...
    pub fn new(name: &str, capacity: usize) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if capacity < MIN_CAPACITY {
            return Err(format!("Capacity {} too small, minimum is {}", capacity, MIN_CAPACITY).into());
        }
        if capacity > MAX_CAPACITY {
            return Err(format!("Capacity {} too large, maximum is {}", capacity, MAX_CAPACITY).into());
        }
        ...
        if element_size > MAX_ELEMENT_SIZE {
            return Err(format!("Element size {} too large, maximum is {}", element_size, MAX_ELEMENT_SIZE).into());
        }
        
        // Safety validation: check for overflow in size calculations
        let data_size = capacity.checked_mul(element_size)
            .ok_or("Integer overflow calculating data size")?;
        if data_size > MAX_TOTAL_SIZE {
            return Err(format!("Data size {} exceeds maximum {}", data_size, MAX_TOTAL_SIZE).into());
        }
        
        // Ensure data section is properly aligned
        let aligned_header_size = (header_size + element_align - 1) / element_align * element_align;
        let total_size = aligned_header_size.checked_add(data_size)
            .ok_or("Integer overflow calculating total size")?;
        
        if total_size > MAX_TOTAL_SIZE {
            return Err(format!("Total size {} exceeds maximum {}", total_size, MAX_TOTAL_SIZE).into());
        }
        ...
        unsafe {
            ...
            (*header.as_ptr()).consumer_count.store(0, Ordering::Relaxed);
            (*header.as_ptr()).sequence_number.store(0, Ordering::Relaxed);
        ...
        publish_log(LogEntry {
            ...
            message: format!("Created topic (capacity: {}, size: {} bytes)", capacity, total_size),
            tick_us: 0,
            ipc_ns: 0,
        });
        ...
        unsafe {
            (*header.as_ptr()).consumer_count.fetch_add(1, Ordering::Relaxed);
        }
    ...
    pub fn open(name: &str) -> Result<Self, Box<dyn std::error::Error>> {
        ...
        if capacity < MIN_CAPACITY || capacity > MAX_CAPACITY {
            return Err(format!("Invalid capacity {} in existing shared memory (must be {}-{})", 
                capacity, MIN_CAPACITY, MAX_CAPACITY).into());
        }
        ...
        let data_ptr = unsafe { 
            ...
            if region.size() < expected_total_size {
                return Err(format!(
                    ...
                    region.size(), expected_total_size
                ).into());
            }
        ...
        let current_head = unsafe {
            (*header.as_ptr()).head.load(Ordering::Relaxed)
        };
                ...
                Ok(_) => {
                    ...
                    unsafe {
                        ...
                        if head >= self.capacity {
                            ...
                            panic!("Critical safety violation: head index {} >= capacity {}", head, self.capacity);
                        }
                        ...
                        if byte_offset + mem::size_of::<T>() > data_region_size {
                            panic!("Critical safety violation: write would exceed data region bounds");
                        }
    ...
    pub fn pop(&self) -> Option<T> {
        ...
        if my_tail >= self.capacity {
            panic!("Critical safety violation: consumer tail {} >= capacity {}", my_tail, self.capacity);
        }
        
        // Validate head position is within bounds
        if current_head >= self.capacity {
            panic!("Critical safety violation: head {} >= capacity {}", current_head, self.capacity);
        }
        ...
        let msg = unsafe {
            // Comprehensive bounds checking
            if my_tail >= self.capacity {
                panic!("Critical safety violation: consumer tail index {} >= capacity {}", my_tail, self.capacity);
            }
            ...
            match header.head.compare_exchange_weak(
                ...
                Ok(_) => {
                    ...
                    unsafe {
                        ...
                        let data_region_size = self.capacity * mem::size_of::<T>();
                        if byte_offset + mem::size_of::<T>() > data_region_size {
                            panic!("Critical safety violation: loan would exceed data region bounds");
                        }
