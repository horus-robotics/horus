...
fn verify_tsc_synchronization() -> Result<u64, String> {
    ...
    if drift < 1000 {
        println!("{}", "(✓ excellent)".bright_green());
    } else if drift < 5000 {
        println!("{}", "(⚠ moderate - expect some variance)".bright_yellow());
    } else {
        println!("{}", "(✗ too large!)".bright_red());
...
fn verify_tsc_synchronization() -> Result<u64, String> {
    println!("\n{}", "TSC Verification:".bright_yellow());
    println!("  • ⚠ Skipped on non-x86_64 platform");
...
fn check_system_state() {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(aslr) = std::fs::read_to_string("/proc/sys/kernel/randomize_va_space") {
            ...
            if aslr_val == "0" {
                println!("  • ASLR: {} {}", "disabled".to_string(), "(✓)".bright_green());
                ok_count += 1;
            } else {
                println!("  • ASLR: {} {}", format!("enabled ({})", aslr_val), "(⚠ may increase variance)".bright_yellow());
        ...
        if let Ok(cmdline) = std::fs::read_to_string("/proc/cmdline") {
            if cmdline.contains("isolcpus") {
                if let Some(isolcpus) = cmdline.split_whitespace()
                    ...
                    println!("  • Core isolation: {} {}", isolcpus.to_string(), "(✓)".bright_green());
                    ok_count += 1;
                }
            } else {
                println!("  • Core isolation: {} {}", "none".to_string(), "(⚠ other processes may interfere)".bright_yellow());
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(output) = std::process::Command::new("ps").arg("aux").output() {
            ...
            if proc_count < 200 {
                ...
                println!("{}", "(⚠ moderate)".bright_yellow());
            } else {
                println!("{}", "(⚠ high - may affect results)".bright_yellow());
    ...
    if warnings.is_empty() {
        println!("  {} System optimized for benchmarking!", "✓".bright_green());
    } else {
...
fn detect_cpu_frequency() -> Result<f64, String> {
    ...
    #[cfg(target_os = "linux")]
    {
        if let Ok(governor) = std::fs::read_to_string(
            ...
            if gov == "performance" {
                println!("{}", "(✓ locked)".bright_green());
            } else {
                println!("{}", "(⚠ not locked - may cause variance)".bright_yellow());
        ...
        if let Ok(turbo) = std::fs::read_to_string(
            ...
            if turbo.trim() == "0" {
                println!("  • {} {}",
                         ...
                         "(⚠ may cause variance)".bright_yellow());
                println!("    Run: echo 1 | sudo tee /sys/devices/system/cpu/intel_pstate/no_turbo");
            } else {
                println!("  • Turbo boost: {} {}", "disabled".to_string(), "(✓)".bright_green());
            }
        }
    }
...
fn main() {
    ...
    match result.measurement_quality.as_str() {
        "high" => {
            println!("  {}", "✓ HIGH QUALITY - All validation checks passed".bright_green().bold());
            ...
            println!("\n  {} These results are suitable for research publication.", "✓".bright_green());
        }
        "medium" => {
            println!("  {}", "⚠ MEDIUM QUALITY - Moderate TSC drift detected".bright_yellow().bold());
            println!("  • TSC verification: PASSED");
            println!("  • CPU frequency: Measured via RDTSC");
            println!("  • TSC drift: {} cycles (moderate variance expected)", result.tsc_drift_cycles);
            println!("\n  {} Usable for performance trends, but note increased variance.", "⚠".bright_yellow());
        }
        "low" => {
            println!("  {}", "⚠ LOW QUALITY - High TSC drift detected".bright_red().bold());
            println!("  • TSC verification: PASSED");
            println!("  • CPU frequency: Measured via RDTSC");
            println!("  • TSC drift: {} cycles (high variance expected)", result.tsc_drift_cycles);
            println!("\n  {} Not recommended for research publication.", "⚠".bright_red());
            println!("  {} Consider running benchmark_setup.sh for better quality.", "→".bright_cyan());
        }
        "invalid" => {
            println!("  {}", "✗ INVALID - Critical validation failures".bright_red().bold());
            ...
            println!("\n  {}", "✗ These results CANNOT be used for research.".bright_red().bold());
            println!("  {} Fix validation issues and re-run.", "→".bright_red());
        }
        _ => {
            println!("  {}", "⚠ UNKNOWN QUALITY STATUS".bright_yellow());
        }
    }
...
fn print_results(all_latencies: &[Vec<u64>], cpu_freq: f64) -> Option<Statistics> {
    ...
    let stats = match calculate_statistics(&all_cycles) {
        Some(s) => s,
        None => {
            println!("\n  {} Statistics calculation failed (empty filtered data)", "✗".bright_red());
            return None;
        }
    };
