/// Critical Production Test Suite for Single-Slot Link Implementation
///
/// This comprehensive test suite validates production readiness with three critical areas:
...
fn print_usage() {
    eprintln!("Critical Production Test Suite for Single-Slot Link");
...
fn test_large_messages_64b() -> TestResult {
    ...
    let consumer_thread = thread::spawn(move || {
        ...
        while consumer_running.load(Ordering::Relaxed) {
            if let Some(msg) = consumer.recv(None) {
                let latency = Instant::now().elapsed().as_nanos() as u64;
    ...
    for i in 0..10000 {
        ...
        if producer.send(msg, None).is_err() {
...
fn test_large_messages_256b() -> TestResult {
    ...
    let producer = match Link::<CameraMetadata>::producer(&topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create producer: {}", e)),
    };

    let consumer = match Link::<CameraMetadata>::consumer(&topic) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create consumer: {}", e)),
    };
...
fn test_large_messages_1kb() -> TestResult {
    ...
    let producer = match Link::<LargeState>::producer(&topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create producer: {}", e)),
    };

    let consumer = match Link::<LargeState>::consumer(&topic) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create consumer: {}", e)),
    };
...
fn test_large_messages_4kb() -> TestResult {
    ...
    let producer = match Link::<ImagePatch>::producer(&topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create producer: {}", e)),
    };

    let consumer = match Link::<ImagePatch>::consumer(&topic) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create consumer: {}", e)),
    };
...
fn test_large_messages_16kb() -> TestResult {
    ...
    let producer = match Link::<PointCloud>::producer(&topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create producer: {}", e)),
    };

    let consumer = match Link::<PointCloud>::consumer(&topic) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create consumer: {}", e)),
    };
...
fn test_mixed_size_workload() -> TestResult {
    ...
    let producer_small = match Link::<SmallMessage>::producer(&topic_small) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create small producer: {}", e)),
    };
    let consumer_small = match Link::<SmallMessage>::consumer(&topic_small) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create small consumer: {}", e)),
    };

    let producer_medium = match Link::<CameraMetadata>::producer(&topic_medium) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create medium producer: {}", e)),
    };
    let consumer_medium = match Link::<CameraMetadata>::consumer(&topic_medium) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create medium consumer: {}", e)),
    };

    let producer_large = match Link::<ImagePatch>::producer(&topic_large) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create large producer: {}", e)),
    };
    let consumer_large = match Link::<ImagePatch>::consumer(&topic_large) {
        Ok(c) => c,
    ...
    let small_thread = thread::spawn(move || {
        while small_running.load(Ordering::Relaxed) {
            if consumer_small.recv(None).is_some() {
    ...
    let medium_thread = thread::spawn(move || {
        while medium_running.load(Ordering::Relaxed) {
            if consumer_medium.recv(None).is_some() {
    ...
    let large_thread = thread::spawn(move || {
        while large_running.load(Ordering::Relaxed) {
            if consumer_large.recv(None).is_some() {
    ...
    while start.elapsed() < Duration::from_secs(2) {
        ...
        if elapsed_us % 5000 < 100 {
            let _ = producer_small.send(SmallMessage::new(small_sent), None);
            small_sent += 1;
        }

        // Medium: 60Hz
        if elapsed_us % 16667 < 100 {
            let _ = producer_medium.send(CameraMetadata::new(medium_sent), None);
            medium_sent += 1;
        }

        // Large: 20Hz
        if elapsed_us % 50000 < 100 {
            let _ = producer_large.send(ImagePatch::new(large_sent), None);
            large_sent += 1;
        }
...
fn run_all_tests() -> TestResult {
    println!("Running COMPLETE CRITICAL PRODUCTION TEST SUITE...\n");
