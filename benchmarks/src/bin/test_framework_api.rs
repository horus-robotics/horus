...
impl SensorNode {
    fn new(topic: &str, counter: Arc<Mutex<u32>>) -> Self {
        Self {
            publisher: Hub::<CmdVel>::new(topic).expect("Failed to create publisher"),
            counter,
            tick_count: 0,
        }
    }
...
impl ControllerNode {
    fn new(input_topic: &str, output_topic: &str, counter: Arc<Mutex<u32>>) -> Self {
        Self {
            subscriber: Hub::<CmdVel>::new(input_topic).expect("Failed to create subscriber"),
            publisher: Hub::<CmdVel>::new(output_topic).expect("Failed to create publisher"),
            counter,
        }
    }
}
...
impl ActuatorNode {
    fn new(topic: &str, counter: Arc<Mutex<u32>>) -> Self {
        Self {
            receiver: Link::<CmdVel>::consumer(topic).expect("Failed to create Link consumer"),
            counter,
        }
    }
}
