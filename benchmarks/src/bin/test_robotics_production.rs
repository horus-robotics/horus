...
fn test_high_frequency_sensor_loop() -> TestResult {
    ...
    let imu_producer = match Link::<Imu>::producer(&imu_topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create IMU producer: {}", e)),
    };

    let imu_consumer = match Link::<Imu>::consumer(&imu_topic) {
        Ok(c) => c,
...
fn test_control_loop() -> TestResult {
    ...
    let sensor_producer = match Link::<Imu>::producer(&sensor_topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create sensor producer: {}", e)),
    };

    let sensor_consumer = match Link::<Imu>::consumer(&sensor_topic) {
        Ok(c) => c,
        Err(e) => return TestResult::failure(format!("Failed to create sensor consumer: {}", e)),
    };

    let command_producer = match Link::<DifferentialDriveCommand>::producer(&command_topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create command producer: {}", e)),
    };

    let command_consumer = match Link::<DifferentialDriveCommand>::consumer(&command_topic) {
        Ok(c) => c,
...
fn test_realistic_robot_pipeline() -> TestResult {
    ...
    // Control Link
    let control_producer = match Link::<DifferentialDriveCommand>::producer(&control_topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create control producer: {}", e)),
    };

    let control_consumer = match Link::<DifferentialDriveCommand>::consumer(&control_topic) {
        Ok(c) => c,
...
fn test_stress_burst() -> TestResult {
    ...
    let producer = match Link::<Imu>::producer(&stress_topic) {
        Ok(p) => p,
        Err(e) => return TestResult::failure(format!("Failed to create stress producer: {}", e)),
    };

    let consumer = match Link::<Imu>::consumer(&stress_topic) {
        Ok(c) => c,
...
fn test_multi_process_ipc() -> TestResult {
    ...
    let producer = match Link::<MotorCommand>::producer(&ipc_topic) {
        Ok(p) => p,
    ...
    let consumer_thread = thread::spawn(move || {
        ...
        let consumer = match Link::<MotorCommand>::consumer(&consumer_topic) {
            Ok(c) => c,
            Err(e) => {
